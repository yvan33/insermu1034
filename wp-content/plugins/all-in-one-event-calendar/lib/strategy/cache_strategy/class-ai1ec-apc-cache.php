...
class Ai1ec_Apc_Cache extends Ai1ec_Cache_Strategy {
	...
	public function write_data( $dist_key, $value ) {
		...
		if ( false !== ( $existing = apc_fetch( $key ) ) ) {
			if ( $value === $existing ) {
				$this->inject_logger()->info(
					'Averting replace of ' . $dist_key .
					' as existing value does not differ'
				);
				return true;
			}
			$store_method = 'apc_store';
		} elseif ( false === function_exists( $store_method ) ) {
			$this->inject_logger()->debug(
				'Current APC version does not support apc_add'
			);
			$store_method = 'apc_store';
		}
		if ( false === $store_method( $key, $value ) ) {
			try {
				...
				$this->inject_logger()->warn(
					'Failed to write ' . $dist_key . ' using ' . $store_method
				);
				$this->inject_logger()->warn( $this );
