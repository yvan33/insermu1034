...
class Ai1ec_Settings_Helper {
	...
	function get_view_options( $view = null ) {
		...
		<div>
			<table>
				...
				<tbody>
					<?php
					$ticket_label = __( 'Display <strong><i class="icon-shopping-cart"></i> Tickets</strong> button for supporting events', AI1EC_PLUGIN_NAME );
					foreach ( $ai1ec_app_helper->view_names() as $key => $name ) {
						...
						<tr>
							...
							<td class="ai1ec-control-table-column">
								<a class="ai1ec-popover-toggle" data-toggle="modal"
	...
	**/
	...
	function get_feed_rows() {
		...
		foreach ( $rows as $row ) {
			$categories = array();
			if ( ! empty( $row->feed_category ) ) {
				foreach ( explode( ',', $row->feed_category ) as $cat_id ) {
			...
			$args          = array(
				...
				'tags'                =>
					stripslashes( str_replace( ',', ', ', $row->feed_tags ) ),
				'feed_id'             => $row->feed_id,
