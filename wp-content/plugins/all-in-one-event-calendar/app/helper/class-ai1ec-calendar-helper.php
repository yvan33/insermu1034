...
class Ai1ec_Calendar_Helper {
	...
	function get_events_between( $start_time, $end_time, $filter, $spanning = false ) {
		...
		$query = $wpdb->prepare(
			...
			"e.show_map, e.contact_name, e.contact_phone, e.contact_email, e.cost, e.ticket_url, " .
			...
			$wpml_where_particle .
			"AND " .
				( $spanning ? "i.end > %d AND i.start < %d "
										: "i.start BETWEEN %d AND %d " ) .
			$filter['filter_where'] .
			$post_status_where .
	...
	/**
	 * Check if the buy ticket button is neabled for the passed view
	 *
	 * @param string $view
	 *
	 * @return boolean
	 */
	public function is_buy_ticket_enabled_for_view( $view ) {
		global $ai1ec_settings;
		$this_buy_ticket_bool = "buy_ticket_{$view}_enabled";
		return ! empty( $ai1ec_settings->views_enabled_ticket_button[$view] );
	}
	...
	protected function _is_all_day( array $event ) {
		...
		$start_plus_day = Ai1ec_Time_Utility::instance()
			->modify_timestamp( '+1 day', $event['start'] );
