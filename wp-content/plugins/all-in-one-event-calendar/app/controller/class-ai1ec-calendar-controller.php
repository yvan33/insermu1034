...
class Ai1ec_Calendar_Controller {
	...
	public function get_calendar_page( Ai1ec_Abstract_Query $request ) {
		...
		if ( false === $generated ) {
			...
			$views_dropdown = $ai1ec_calendar_helper
				->get_html_for_views_dropdown( $dropdown_args );
			$subscribe_buttons = '';
			if ( ! $ai1ec_settings->turn_off_subscription_buttons ) {
				$subscribe_buttons = $ai1ec_calendar_helper
					->get_html_for_subscribe_buttons( $view_args );
			}
			if (
				...
				$content = $ai1ec_view_helper->get_theme_view(
					'calendar.php',
					$page_args
				);
